...
class Books():
    ...
    def __init__(self):
        ...
        self.average_rating['ratingCount'] = \
            pd.DataFrame(self.ratings_explicit.groupby('ISBN')['bookRating'].count())
        ...
        #self.ratings_explicit = \
         #   self.ratings_explicit[self.ratings_explicit['userID'].isin(counts1[counts1 >= 5].index)]
        ...
        isbn_col = [i for i in range(1000)]
        isbn_col = pd.DataFrame(isbn_col)
        isbn_col.columns =['ISBN']
        self.average_rating = pd.concat([self.average_rating,isbn_col],axis=1)
        #print(self.average_rating[25:30])
        
    def Top_Books(self, n=6, RatingCount = 15, MeanRating = 4):
        
        BOOKS = self.books.merge(self.average_rating, how = 'right',on='ISBN')
        
        M_Rating = BOOKS.loc[BOOKS.ratingCount >= \
                             RatingCount].sort_values('MeanRating', ascending = False).head(n)
        
        H_Rating = BOOKS.loc[BOOKS.MeanRating >= \
                             MeanRating].sort_values('ratingCount', ascending = False).head(n)
...
class SVD(Books):
    ...
    def __init__(self, n_latent_factor = 50):
        ...
        self.ratings_mat = self.ratings_explicit.pivot(index="userID",\
                                                       columns="ISBN", values="bookRating").fillna(0)
        ...
        self.User_lookup = dict(zip(range(1,len(self.explicit_users)+1),self.explicit_users))
    ...
    def scipy_SVD(self):
        ...
        self.predictions = pd.DataFrame(X_pred, columns = \
                                        self.ratings_mat.columns, index = self.ratings_mat.index)
...
class KNN(Books):
    ...
    def __init__(self, n_neighbors = 10):
        ...
        self.ratings_mat = self.ratings_explicit.pivot(index="ISBN",\
    ...
    def Recommend_Books(self, book, n_neighbors = 6):
